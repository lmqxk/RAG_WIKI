...
| 策略 | 优点 | 缺点 | 本项目结论 |
| --- | --- | --- | --- |
| 固定长度滑窗 | 实现简单、长度均匀 | 切断条款语义，引用定位差 | 仅作为长文本兜底（句末标点拆分 + 180 字重叠） |
| 递归字符分割（LangChain 式） | 通用 | 不理解规范文体结构 | 不采用 |
| 结构感知切分（本项目） | 条款/章节/表格语义完整，条款号锚点保留 | 依赖解析质量 | **采用**：按条款号、章节标题、表格类型切分 |

规范文本的天然粒度是"条款"：一个条款 = 一个语义单元 = 一个引用锚点。结构感知切分让 `chunk.clause_no`、`chapter_path`、页码成为可检索、可引用的元数据。
...
`PageBlock.images` 不直接写入 `chunks` 表的独立字段，但表格文本里仍包含图片提示文本（如 `图片文件：images/f078.jpg`）；资料预览从 `normalized.json` 读取结构化 `images` 选择匹配问题图片。

## 长文本处理

超过长度上限的文本按中文句末标点拆分，并保留少量重叠内容，避免跨句信息断裂。

默认参数：

- 单块长度：约 1400 字符（保证嵌入模型 4096 token 窗口内单块完整）
- 重叠长度：约 180 字符
...
回答生成时优先信任 `normative`，但仍保留说明区作为解释性证据。
...
修改了切块代码但还没有重建索引时，可以直接从 `normalized.json` 用当前代码预览 fresh 切块结果：
...
| 模式 | 含义 |
| --- | --- |
| `--document-id` | 查看 SQLite 当前已入库的 chunk，代表真实检索正在使用的索引 |
| `--fresh --parsed-dir` | 不读数据库，直接用当前 `chunking.py` 从 `normalized.json` 重新切块，适合验证新规则 |

如果 fresh 结果正确但数据库结果还是旧样式，需要重启后端并调用 `reindex` 重建该文档索引。

## 运维思考

| 维度 | 考量 |
| --- | --- |
| 时延 | 切分是纯正则，毫秒级，从不成为瓶颈 |
| 检索质量联动 | 块长 1400 字符是"引用粒度"与"嵌入质量"的折中：太短语义不完整，太长向量表征被稀释、引用定位变粗 |
| 维度预算 | 全库约 2000+ chunks；1024 维 float32 单向量约 4KB，Qdrant 向量数据约 10MB 量级，内存与磁盘压力可忽略 |
| 变更成本 | 切分规则改动只需 `reindex`（复用解析产物）；chunk ID 由切分逻辑生成，Wiki 锚点按 chunk ID 关联，重新切分会改变 ID，需要同步刷新 Wiki |
...
```powershell
cd E:\lmq\RAG_ZB
E:\lmq\RAG_ZB\.tools\uv\bin\uv.exe run --project backend pytest backend\tests\test_chunking_tables.py -q
```
