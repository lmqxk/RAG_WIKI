...
> 条目按时间倒序排列：最新修改在最上，新条目插在文件开头。
> 提交前必须先迭代本文件（规范见 `AGENTS.md`）。

## 2026-09-04 chore: 清理根目录旧文件，XMind 全景图合并入《系统架构》

- 表层问题：根目录残留 `mermaid-diagram.svg`（旧架构图：BGE 向量/Jina 重排/MinerU 管线，全部过时且无人引用）与 `规范智能问答系统-当前架构.xmind`（7 月 30 日版本，内容落后当前架构）；且转写出的《系统全景脑图》与《系统架构》存在内容重合。
- 根因：架构演进后旧图未同步清理；xmind 为二进制格式无法随代码演进维护。
- 修复：删除两个过时文件；xmind 内容（九板块全景树）合并入 `docs/01-系统总览/系统架构.md`——新增「全景地图」章节（浓缩树状图，产品交互/演进路线等架构.md 原缺失板块补入）与「模块文档索引」表；运维思考表同步更新（TTFT 6.5s、全链路并发实测）；独立《系统全景脑图.md》删除。根目录临时脚本 `test.py`（火山方舟 API 连通性验证）一并移除。`start.cmd` 经确认仍为有效本地开发入口（AGENTS.md 环境备忘引用，scripts/run.py 存在），保留。
- 同类回扫：README.md 目录结构移除 xmind 条目；docs/README.md 索引同步；全库搜索无其他残留 svg/xmind 旧架构文件、无指向已删脑图文档的死链。
- 验证：根目录无孤儿架构文件；docs 索引与实际文件一致；合并后《系统架构》链接抽查可跳转。

## 2026-09-04 feature: 全链路阶梯压测（1/5/20/100 并发）与运维 Q\&A 扩充

- 需求：技术选型问答文档需要全链路并发容量数据（此前仅有 5 并发一组），并补 Uvicorn/FastAPI 关系说明。

- 方案：`scripts/load_test.py` chat 场景阶梯压测；Q8 重写为「FastAPI 是写什么、Uvicorn 是怎么跑、ASGI 是插座标准」分层讲解（含餐厅类比与请求流转图）；Q9 换为全链路并发表。

- 实现：实测 1 并发基线 7.8s；20 并发 1.87 req/s（p50 8.4s，112 请求 100% 成功）；100 并发 4.58 req/s（p50 19.7s，412 请求 100% 成功）——并发 20→100 涨 5 倍吞吐仅涨 2.45 倍（天花板在每次问答 2 次云端 LLM 调用），但延迟劣化平滑无雪崩；Q9 同时记录 Planner thinking 修复带来的 TTFT 15s→6.5s 过程。

- 同类扩展：Q8 补 ASGI 协议规范 vs 服务器实现的辨析（uvloop/httptools 在 uvicorn 内的位置）；Q9 扩容路径更新为含已完成项的优先级列表。

- 验证：压测工具原生输出统计与文档表格逐项一致；412 请求 0 失败。

## 2026-09-04 fix: Planner 思考链吃穿 max\_tokens 预算，规划静默降级且 TTFT 虚高

- 表层问题：问答 TTFT 13.4\~15.3s，其中 planner\_ms 占 11.2\~13.2s（65%+）；且 LLM 检索计划时好时坏，多数请求实际走了确定性回退步骤（静默降级）。

- 根因：A/B 实验定位——deepseek-v4-flash 默认开启 thinking，reasoning token 计入 max\_tokens=1200 预算且长度随机波动（实测 525\~1941）；预算被 reasoning 吃穿时 finish\_reason=length，JSON 截断甚至 content 为空，`_loads_json_object` 解析失败后 `agent.py` 静默回退 `_initial_steps`——12 秒规划税白付。调小 max\_tokens 方向证伪：600/400 预算下输出直接为空。

- 修复：`agent.py` planner 请求增加 `"thinking": {"type": "disabled"}`。规划是 temperature=0 的填空式 JSON 任务（输入侧分类/聚焦已由确定性规则完成，输出侧固定 schema 三工具），思考链只付延迟成本不付质量收益——实测同一问题开/关 thinking 生成的 5 步计划内容一致，耗时 9.6\~24.9s vs 3.2\~5.0s。

- 同类回扫：检查 `providers.py` 回答生成调用（流式、用户预期深度回答，thinking 保留合理，暂不动）；测试期间发现宿主机直接 sqlite3 连接挂载卷 rag.db 会触发 Docker Desktop 文件共享锁，导致容器内 SQLite 全部 `unable to open database file`，`docker restart rag-zb-backend` 恢复——运维教训写入硬约束：容器运行时不从宿主机直接打开 storage/ 下 SQLite。

- 验证：9 个 agent 测试通过；重建后端镜像后线上实测同一批问题 planner\_ms 11.2\~13.2s → 4.4\~5.9s，TTFT 15.3s → 6.5s，total\_ms 21s → 9.2s，计划稳定 2\~4 步完整 JSON 不再截断。
...
- 实现：Q2 增加 L1/L2/L3 超长条款三级处理表（1400/180/800/650 参数来自 chunking.py 实测）、真实 chunk 元数据示例（8.4.5 与 8.4.5#2 续块、表格 chunk，取自 storage/rag.db 真实数据）、条文说明单向分区状态机小节；Q6 重构为五层防线（提示词七规则表 / 结构化证据 JSON / temperature 0.1 / \_used\_citations 校验 / 抽取式降级）；登记 docs/README.md 索引。
...
- 实现：`chunking.py` 增加 `FRONT_MATTER_RE`、`WATERMARK_RE`、front\_matter 状态机、`parent_id` 解析、超长句硬切分。
